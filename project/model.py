...
class Model:
    def __init__(self, ds, weights):
        self.ds = ds
        self.create_model(weights)
    ...
    def macro_f1(target_y, predicted_y, thresh=0.5):
        ...
        tp = tf.cast(tf.math.count_nonzero(y_pred * target_y, axis=0),
                     tf.float32)
        fp = tf.cast(tf.math.count_nonzero(y_pred * (1 - target_y), axis=0),
                     tf.float32)
        fn = tf.cast(tf.math.count_nonzero((1 - y_pred) * target_y, axis=0),
                     tf.float32)
    ...
    def create_model(self, weights):
        ...
        with mirrored_strategy.scope():
            ...
            self.model.compile(
                ...
                loss=[
                    lambda y_true, y_pred: Model.macro_double_soft_f1(
                        y_true, y_pred)
                ],
                metrics=[
                    ...
                    tf.keras.metrics.AUC(curve='PR',
                                         ...
                                         multi_label=True), Model.macro_f1,
    ...
    def predict(self, images):
        ...
        for prediction in predictions:
            ...
            decoded_prediction['gender'] = self.ds.one_hot_decoding[np.argmax(
                prediction[0:2])]
            decoded_prediction['articleType'] = self.ds.one_hot_decoding[
                np.argmax(prediction[2:17]) + 2]
            decoded_prediction['baseColour'] = self.ds.one_hot_decoding[
                np.argmax(prediction[17:28]) + 17]
            decoded_prediction['season'] = self.ds.one_hot_decoding[
                np.argmax(prediction[28:32]) + 28]
            decoded_prediction['usage'] = self.ds.one_hot_decoding[
                np.argmax(prediction[32:]) + 32]
